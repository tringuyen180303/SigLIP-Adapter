...
data:
  root_path: "/kaggle/input/eurosat-dataset/EuroSAT"  # path to Food‑101 directory
  dataset_name: "EuroSAT"
  shots: 4               # few‑shot images per class for cache keys
  overwrite: false
  cache_root: "./feature_cache_eurosat"

model:
  name: "SigLIP"
  load_cache: true
  cache_dir: "./cache_eurosat"
  augment_epoch: 1
...
class DatasetBase:
    ...
    def generate_fewshot_dataset(self, data, num_shots, seed=42):
        ...
        label_to_items = {}
        for item in data:
            label = item.label if hasattr(item, 'label') else item[1]
    ...
    def get_num_classes(self, data_source):
        label_set = set(item.label if hasattr(item, 'label') else item[1] for item in data_source)
        return max(label_set) + 1

    def get_lab2cname(self, data_source):
        container = set((item.label, item.classname) if hasattr(item, 'label') else (item[1], item[2])
                        for item in data_source)
        mapping = {label: cname for label, cname in container}
        labels = sorted(mapping.keys())
        classnames = [mapping[lbl] for lbl in labels]
